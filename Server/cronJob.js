...
const cronJob=async () => {
    ...
    for (i = 0; i < allWebsites.length; i++) {
      ...
      console.log(weburl," currentStatus:",allWebsites[i].isActive," NewStatus: ",isActive);
      ...
      if(!isActive && allWebsites[i].isActive){ 
          ...
          transport.sendMail({
              ...
              html:`Your Website <b> ${allWebsites[i].url} </b> is down . As we checked on ${new Date().toLocaleDateString("en-in")}`,
          });
